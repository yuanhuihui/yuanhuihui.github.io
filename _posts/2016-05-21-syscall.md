...
**3.1：** 用户程序通过软中断swi指令切入内核态，执行vector_swi处的指令。`vector_swi`在文件`/kenel/arch/arm/kernel/entry-common.S`中定义:
...
sys_kill()定义在内核源码找不到直接定义，而是通过SYSCALL_DEFINE。
