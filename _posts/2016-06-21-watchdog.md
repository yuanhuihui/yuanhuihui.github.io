...
- Watchdog运作过程，当阻塞时间超过1分钟则触发一次watchdog，视情况可能会导致上层重启；

##  三、Watchdog监测

当调用Watchdog.getInstance().start()时，则进入线程“watchdog”的run()方法。
...
该run()方法分成两部分，这前半部分用于监测是否触发超时，当触发超时则进入后半部分[小节4.1]输出各种信息。
...
postAtFrontOfQueue(this)，该方法输入参数为Runnable对象，根据[消息机制](http://gityuan.com/2015/12/26/handler-message-framework/#postatfrontofqueue)，
最终会回调HandlerChecker中的run方法，该方法会循环遍历所有的Monitor接口，具体的服务实现该接口的monitor()方法。

如果handler消息池处理得慢，或者前面有太多消息要处理导致的watchdog.
...
### 4.1 run
    ...
    public void run() {
        while (true) {
            ...
            //第二次已追加的方式，输出system_server和3个native进程的栈信息【见小节4.2】
...
输出system_server和`/system/bin/mediaserver`,`/system/bin/sdcard`,`/system/bin/surfaceflinger`这3个native进程的traces信息。

### 4.3 dumpKernelStackTraces
...
关于dropbox已在[dropBox源码篇](http://gityuan.com/2016/06/12/DropBoxManagerService/)详细讲解过，输出文件到/data/system/dropbox，比如system_server_watchdog.
...
Watchdog监控的线程有：`DEFAULT_TIMEOUT默认为60s`，调试时才为10s方便找出潜在的ANR问题。
...
|ActivityManager|AMS.MainHandler|AMS构造函数指定|
|PowerManagerService|PMS.PowerManagerHandler|PMS.onStart()|

### 5.2 Monitor方式

能够被Watchdog监控的系统服务都实现了Watchdog.Monitor接口，并实现其中的monitor()方法。 
系统中实现该接口类主要有：
...
2. WD.dumpKernelStackTraces，输出kernel栈信息;
3. doSysRq, 触发kernel来dump所有阻塞线程，输出所有CPU的backtrace到kernel log;
4. dropBox，输出文件到/data/system/dropbox.
5. 杀掉system_server，级联zygote进程自杀，触发重启上层framework。
