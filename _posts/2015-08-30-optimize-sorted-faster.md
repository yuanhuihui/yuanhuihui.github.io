...
> **遍历已排序和未排序的数组，时间复杂度都是O(n)，为什么遍历已排序比未排序数组速度更快？**
...
起初，我认为这可能仅仅是语言或者编译器的反常的原因。 因此我尝试用JAVA实现。
...
![railroad junction](/images/optimize-1/1.jpg)
...
![railroad junction](/images/optimize-1/2.png)
...
----------
这就是分支预测。我承认这不是最好的比喻，因为车可能只是信号和标志的方向。但在计算机中，处理器在最后一刻之前并不知道执行哪个分支。  
...
进一步阅读， ["Branch predictor" article on Wikipedia.](http://en.wikipedia.org/wiki/Branch_predictor)
...
- 当data[c]大于128时，`int t = (data[c] - 128) >> 31`，根据带符号的右移位运算法则，得到t=0，则~t=-1;
从而~t & data[c] = data[c]，故等效sum += data[c]; 
...
> 本文翻译过来的，原来来自[stackoverflow](http://stackoverflow.com/questions/11227809/why-is-processing-a-sorted-array-faster-than-an-unsorted-array)